...
RUN apt update && apt install -y git && apt-get clean && rm -rf /var/lib/apt/lists/* && \
  ...
  cd / && rm -rf /workspace && mkdir -p /workspace && mv /dllm/v2/* /workspace && rm -rf /dllm && \
  cd /workspace && sed -i 's@server_port=10086, share=True@server_name="0.0.0.0", server_port=7860@' app.py
